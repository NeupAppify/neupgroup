...
export async function getAnalyticsContext() {
  ...
  return { traceId, contextId, signedContextId: signContextId(contextId), projectId: "cmtx8ocvh0000bup9m18a09z1" };
}
...
export async function logPageActivity(contextId: string, pageUrl: string): Promise<void> {
  ...
  try {
    await fetch("https://neupgroup.com/analytics/bridge/api.v1/activity?project=cmtx8ocvh0000bup9m18a09z1", {
      method: "POST",
...
export async function logActivity(activity: string, data?: Record<string, unknown>): Promise<void> {
  ...
  const requestHeaders = await headers();
  await fetch(
    "https://neupgroup.com/analytics/bridge/api.v1/activity?project=cmtx8ocvh0000bup9m18a09z1",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
